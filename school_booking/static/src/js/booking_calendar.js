        ...
        return new Model("res.users")
            ...
            .done(function (result) {
                ...
                new Model("calendar.contacts")
                    .query(["partner_id"])
                    .filter([["user_id", "=", self.dataset.context.uid]])
                    .all()
                    ...
                    .done(function () {
                        // Get my assets
                        new Model("school.calendar.assets")
                            .query(["asset_id"])
                            .filter([["user_id", "=", self.dataset.context.uid]])
                            .all()
                            .then(function (result) {
                ...
                new Model("school.asset")
                    ...
                    .done(function (result) {
                        ...
                        self.ds_message = new data.DataSetSearch(
                            self,
                            "school.calendar.assets"
                        );
                        defs.push(
                            self.ds_message.call("create", [{asset_id: asset_id}])
                        );
                    })
            );
        ...
        _do_search: function (domain, context, _group_by) {
            ...
            this.asset_event_source = {
                events: function (start, end, timezone, callback) {
                    ...
                    var asset_ids = $.map(self.all_filters, function (o) {
                        if (o.is_checked) {
                            return o.asset_value;
                        }
                    });
                    if (!_.isEmpty(asset_ids)) {
                        event_domain = new data.CompoundDomain(event_domain, [
                            "|",
                            ["asset_ids", "in", asset_ids],
                            ["room_id", "in", asset_ids],
                        ]);
                    }
                    ...
                    self.dataset
                        ...
                        .done(function (events) {
                            ...
                            if (
                                self.current_start.getTime() !=
                                    start.toDate().getTime() ||
                                self.current_end.getTime() != end.toDate().getTime()
                            ) {
                            ...
                            if (self.asset_event_source !== current_event_source) {
                                console.log(
                                    "Consecutive ``do_search`` called. Cancelling."
                                );
                                return;
                            }
                            ...
                            if (!self.useContacts) {
                                ...
                                if (self.sidebar) {
                                    ...
                                    events = $.map(events, function (e) {
                                        var key =
                                            color_field.type == "selection"
                                                ? e[self.color_field]
                                                : e[self.color_field][0];
                                        if (
                                            _.contains(self.now_filter_ids, key) &&
                                            self.all_filters[key].is_checked
                                        ) {
                                            return e;
                                        }
                                        return null;
                            ...
                            all_attendees = _.chain(all_attendees)
                                .flatten()
                                .uniq()
                                .value();
                            ...
                            if (self.avatar_title !== null) {
                                new Model(self.avatar_title)
                                    ...
                                    .done(function () {
                                        return self
                                            .perform_necessary_name_gets(events)
                                            .then(callback);
                                    });
                            } else {
