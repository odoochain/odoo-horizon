...
class ProgramApprouval(models.Model):
    ...
    valuation_followup_ids = fields.Many2many('school.valuation_followup', 'school_valuation_approuval_rel', 'approuval_id', 'valuation_id', string='Valuations')
    ...
    def _on_update_individual_bloc_ids(self):
        for rec in self:
            ...
            rec.valuation_followup_ids = self.env['school.valuation_followup'].search([['individual_program_id','in',all_program_ids.ids],['state','!=','0_valuated']])
...
class IndividualBloc(models.Model):
    ...
    def action_add_comment_approuve_bloc(self):
        self.ensure_one()
        comments = self.env['school.bloc_approuval'].search([['approuval_id','=',self._context.get('approuval_id')],['bloc_id','=',self.id]])
        if comments :
            pass
        else :
            return {
                ...
                'view_mode': 'form',
                'search_view_id' : (self.env.ref('school_program_approuval.view_approuve_blocs_filter').id,),
                'views': [[self.env.ref('school_program_approuval.bloc_approuval_form').id,'form']],
                'context': {'approuval_id':self.id},
            }
