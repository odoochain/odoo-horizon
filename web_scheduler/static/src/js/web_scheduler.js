            ...
            if (self.scheduler_group) {
                var bindCalendarButton = function (selector, arg1, arg2) {
                    self.$buttons.on(
                        "click",
                        selector,
                        _.bind(self.$calendar.fullCalendar, self.$calendar, arg1, arg2)
                    );
                };
                bindCalendarButton(
                    ".o_calendar_button_timeline",
                    "changeView",
                    "timelineDay"
                );
            }
                ...
                var ret = $.extend(this._super(), {
                    ...
                    viewRender: function (view) {
                        ...
                        self.$calendar.fullCalendar(
                            ...
                            Math.max(
                                290,
                                parseInt(self.$(".o_calendar_view").height()) - 30
                            )
                        );
                        ...
                        setTimeout(function () {
                            ...
                            $fc_view
                                ...
                                .css(
                                    "width",
                                    width > $fc_view.width() ? width : "100%"
                                ); // 100% = fullCalendar default
                        }, 0);
                    },

                    resources: function (callback) {
                        var scheduler_group = self.fields[self.scheduler_group];
                        new Model(scheduler_group.relation)
                            .query(["name"])
                            .filter(self.scheduler_domain)
                            .all()
