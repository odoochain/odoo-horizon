...
class Annexe5Xlsx(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, saturn):
        ...
        for bloc_id in saturn.bloc_ids:
            ...
            hist_id = self.env['school.student_annexe5_entry'].search([('year_id','=',year_id.id),('student_id','=',bloc_id.student_id.id)])
            if hist_id:
                sheet.write(i, 49, hist_id.activite)
                sheet.write(i, 50, hist_id.inscription)
                sheet.write(i, 51, hist_id.code_saturn)
                sheet.write(i, 52, hist_id.type)
                sheet.write(i, 53, hist_id.resultat)
                sheet.write(i, 54, hist_id.pae_num)
                sheet.write(i, 55, hist_id.pae_den)
            year_id = year_minus_2
            hist_id = self.env['school.student_annexe5_entry'].search([('year_id','=',year_id.id),('student_id','=',bloc_id.student_id.id)])
            if hist_id:
                sheet.write(i, 56, hist_id.activite)
                sheet.write(i, 57, hist_id.inscription)
                sheet.write(i, 58, hist_id.code_saturn)
                sheet.write(i, 59, hist_id.type)
                sheet.write(i, 60, hist_id.resultat)
                sheet.write(i, 61, hist_id.pae_num)
                sheet.write(i, 62, hist_id.pae_den)
            year_id = year_minus_1
            hist_id = self.env['school.student_annexe5_entry'].search([('year_id','=',year_id.id),('student_id','=',bloc_id.student_id.id)])
            if hist_id:
                sheet.write(i, 63, hist_id.activite)
                sheet.write(i, 64, hist_id.inscription)
                sheet.write(i, 65, hist_id.code_saturn)
                sheet.write(i, 66, hist_id.type)
                sheet.write(i, 67, hist_id.resultat)
                sheet.write(i, 68, hist_id.pae_num)
                sheet.write(i, 69, hist_id.pae_den)
            #sheet.write(i, 70, u'date inscription')
            #sheet.write(i, 76, u'REGULIER')
            #sheet.write(i, 77, u'IRREGULIER')
            #sheet.write(i, 78, u'FINANCABLE')
