                ...
                if(self.edit_mode) {
                    ...
                    new Model('calendar.event').call('create', [{
                        ...
                        if(error.data.exception_type == "validation_error"){
                            Dialog.alert(null, _t(error.data.message));
                        }
                    }).then(function (id) {
	    ...
	    ajax.jsonRpc('/booking/events', 'call', {
                ...
                events.forEach(function(evt) {
                    self.events.push({
                        ...
                        'id': evt.id,
                        'resourceId':evt.room_id[0],
                        'color': evt.recurrency ? '#00838f' : session.uid == evt.user_id[0] ? '#0d47a1' : '#fdd835', 
                    });
                });
                //console.log([start, end, events])
                callback(self.events);
...
var Browser = Widget.extend({
    ...
    start: function() {
        ...
        self.main_modal = this.$('#main-modal').parent().modal();
