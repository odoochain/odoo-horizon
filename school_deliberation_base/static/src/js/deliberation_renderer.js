    ...
    var DeliberationRenderer = BasicRenderer.extend({
        ...
        _renderHeader : function () {
            ...
            var $img = $('<img>',{ 
                ...
                'src' : this._getImageURL('res.partner','image_medium',record.student_id.data.id,'student picture'),
            ...
            if(this.state.model=='school.individual_bloc') {
                ...
                for(var i =0; i < record.acquired_ind_course_group_ids.data.length; i++) {
                    ...
                    $tbody.append(`<tr class="course_group">
                        ...
                        <td>
                            ${course_group.data.year_id.data ? course_group.data.year_id.data.display_name + ' - ' : ''}${course_group.data.title}${course_group.data.responsible_id ? ' <span class="text-muted">- ' + course_group.data.responsible_id.data.display_name : ''}</span> <span class="text-muted" style="font-size: 60%;">(${course_group.data.uid})</span>
                        </td>
                        <td class="text-right">
                            ${course_group.data.final_result_disp}
                        </td>
